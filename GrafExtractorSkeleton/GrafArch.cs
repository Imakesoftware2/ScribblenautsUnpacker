...
namespace GrafExtractorSkeleton
{
    ...
    public sealed class GrafArch : IDisposable
    {
        ...
        public GrafArch(string archPath)
        {
            ...
            {
                Console.WriteLine("Can't open packager index. Trying packager index for code instead.");
                try
                {
                    packagerPath = Path.Combine(dataPath, "pmindex_for_code.xml");
                    xmlReader = XmlReader.Create(packagerPath, new XmlReaderSettings() { IgnoreComments = true, IgnoreWhitespace = true });
                }
                catch
                {
                    Console.WriteLine("Can't open packager index. File names may not be available, and directory structure will not be available.");
                }
            }
        ...
        public void ReadDirectory()
        {
            ...
            if (grafStream.Length > fileCount * 18 + 4) isUnmasked = true;
            ...
            for (int i = 0; i < fileCount; ++i)
            {
                ...
                entry.NameHash = isUnmasked ? grafReader.ReadInt32() : i;
                ...
                // Unmasked has a big region of empty entries for some reason
                if (!(entry.NameHash == 0 && entry.CompressedLength == 0 && entry.Length == 0)) files.Add(entry);
            ...
            if (xmlReader != null)
            {
                ...
                while (xmlReader.Read())
                {
                    ...
                    int index = int.Parse(xmlReader.GetAttribute(isUnmasked ? "namehash" : "index"));
                    try
                    {
                        FileEntry entry = files.Find(e => e.NameHash == index);
                    ...
                    catch (NullReferenceException)
                    {
                        if (!isUnmasked)
                        {
                            Console.WriteLine("Packager index referred to index {0}, which is not in the current package set. File names may be incorrect.", index);
                        }
                        else
                        {
                            Console.WriteLine("Packager index referred to hash {0} (path {1}), which is not in the current package set.", index, xmlReader.GetAttribute("name"));
                        }
                    }
                }
                foundNames = true;
            ...
            if (!foundNames)
            {
                Console.WriteLine("Attempting to get names from 1s...");
                try
                {
                    BinaryReader sbr = new BinaryReader(File.OpenRead(Path.Combine(dataPath, "1s")));
                    int nStrings = sbr.ReadInt32();
                    for (int i = 0; i < nStrings; ++i)
                    {
                        string str = sbr.ReadString();
                        int ind = sbr.ReadInt32();
                        try
                        {
                            FileEntry entry = files[ind];
                            entry.FileName = str;
                        }
                        catch (IndexOutOfRangeException)
                        {
                            Console.WriteLine("1s referred to index {0}, which is not in the current package set. File names may be incorrect.", ind);
                        }
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine("Unable to read 1s. File names will not be available. ({0})", ex.Message);
                }
            }
            ...
            for (int i = 0; i < files.Count; ++i)
            {
                ...
                if (string.IsNullOrEmpty(entry.FileName))
                    entry.FileName = entry.NameHash + ".bin";
            }
            ...
            for (int i = 0; i < archNameCount; ++i)
            {
                ...
                if (File.Exists(pakPath))
                {
                    ...
                    pakStreams[i] = File.Open(pakPath, FileMode.Open, FileAccess.ReadWrite);
                }
            }
        ...
        public void ExtractFile(FileEntry entry, Stream extractStream)
        {
            ...
            if (entry.CompressionLevel > 0) targetArch = StreamUtils.OpenDeflateDecompressionStreamCheap(targetArch, true); // I'm lazy. Assign deflate stream to target so I only have to write copy statement once.
